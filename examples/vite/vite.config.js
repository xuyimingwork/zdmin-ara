...
function myApiTransformer({
  method, path, doc, base
}) {
  return {
    imports: [
    ...
    ...doc.name.startsWith('gpx-') ? { name: camelCase(basename(path)) } : {},
    ...doc.name.startsWith('gpx-document') && base.output.startsWith('gpx-document/')  ? { 
